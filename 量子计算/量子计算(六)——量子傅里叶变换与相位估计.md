...
>笔者个人的理解是：对于控制位而言，其包含的$| 0 \rangle$态与$| 1 \rangle$态并不是平权的。例如，当控制位为$1$时，那么在作用之后也就恰好是$e^{i2\pi\theta}\beta| 1 \rangle| \psi \rangle$，此时产生的确实是全局相位；而控制位为$0$时，则不会引入相位改变。于是由于叠加态原理，控制位的两个计算基态间的相位差发生改变，也就导致了相对相位的产生或改变。
