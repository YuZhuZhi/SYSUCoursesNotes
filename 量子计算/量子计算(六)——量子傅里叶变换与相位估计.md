...
那么，假设控制位处于量子态$| \varphi \rangle=\alpha| 0 \rangle+\beta| 1 \rangle$而受控位恰为$U$的特征向量$| \psi \rangle$。经过$CU$作用之后：
...
现在让我们考虑当受控位不是特征向量的情况。由于$U$是酉矩阵，其各特征向量自然是正交的，因此任意(等量子位数的)量子态$| \phi \rangle$都可以表示为其特征向量之线性和：
...
$$CU(| \varphi \rangle| \psi \rangle)=\alpha| 0 \rangle| \psi \rangle+\beta\displaystyle\sum w_ke^{i2\pi\theta_k}| 1 \rangle| \psi_k \rangle$$
...
在现实世界中，量子门往往仅是个黑箱——这意味着我们并不知道这个量子门的具体形式，也就更不必提求出它的特征值了。而相位估计就是为了解决这个问题。

显然对于特征向量来说，无论是否利用相位反冲，量子门$U$都只能引入一个对观测概率不产生影响的相位，因此这对我们估计量子门的特征值没有任何作用。但是注意到：我们之前介绍的量子傅里叶变换，就是让计算基态进入具有相对相位的叠加态；而量子电路可逆，相反地也自然能将相对相位转换为计算基态——而计算基态被测量到的概率就是$100\%$！这就给了我们相位估计的思路：只要利用相位反冲，让相位作用到控制位上，最后让控制位逆傅里叶变换一次即可。

### 2.线路构建

为了分析简便，取受控位为$U$的特征向量$| \psi \rangle$。在相位反冲之前，我们要让控制位都进入叠加态——否则要么不引入相位、要么引入全局相位。为此首先让控制位从初始零态做一次量子傅里叶变换：
...
现在考虑如何使$U$的相位反冲到控制位上。我们再次将特征值$e^{i2\pi\theta}$中的$\theta$也写为二进制小数形式：

$$\theta=0.\theta_1\theta_2...\theta_{n-1}\theta_n...$$

因此$U^2$的特征值就会是$(e^{i2\pi\theta})^2=e^{i2\pi2\theta}=e^{i2\pi0.\theta_2\theta_3...\theta_{n-1}\theta_n...}$，同理$U^{2^{k}}$中的$\theta$值为$0.\theta_{k}\theta_{k+1}...$。这样我们就构造出与量子傅里叶变换中各量子位相对相位相似的排列了。因此这个量子线路将如下构造：
